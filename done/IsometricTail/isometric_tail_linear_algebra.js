class DiamondBoard {
    ...
    drawDiamond(canvas, diamond) {
        ...
        this.draw3DEffect(canvas, diamond);
    ...
    drawDiamondContent(canvas, diamond) {
        var ctx = canvas.getContext("2d");
        var offsetX = this.diagonalNum * this.width / 2;
        var height_effect3d = this.height / 10;
        var pt0 = this.getDiamondCoord(diamond.x, diamond.y);
        var pt2 = this.getDiamondCoord(diamond.x + 1, diamond.y + 1);
        var length = 0;
        ctx.save();
        ...
        ctx.save();
        ctx.beginPath();
        ctx.translate((pt0.i + pt2.i) / 2 + offsetX, (pt0.j + pt2.j) / 2 - height_effect3d);
        if (this.height > this.width) {
            ctx.scale(this.width / this.height, 1);
            ctx.arc(0, 0, length, 0, Math.PI * 2);
        }
        ...
        ctx.beginPath();
        ctx.moveTo((pt1.i + pt3.i) / 2 + offsetX - length, (pt0.j + pt2.j) / 2);
        ctx.lineTo((pt1.i + pt3.i) / 2 + offsetX + length, (pt0.j + pt2.j) / 2);
        ctx.lineTo((pt1.i + pt3.i) / 2 + offsetX + length, (pt0.j + pt2.j) / 2 - height_effect3d);
        ctx.lineTo((pt1.i + pt3.i) / 2 + offsetX - length, (pt0.j + pt2.j) / 2 - height_effect3d);
        ctx.lineTo((pt1.i + pt3.i) / 2 + offsetX - length, (pt0.j + pt2.j) / 2);
        ctx.fillStyle = diamond.drawColor;
        ctx.fill();
    ...
    draw3DEffect(canvas, diamond) {
        ...
        ctx.beginPath();
        ctx.moveTo(offsetX + pt1.i, pt1.j);
        ctx.lineTo(offsetX + pt2.i, pt2.j);
        ctx.lineTo(offsetX + pt2.i, pt2.j + this.height / 2);
        ctx.lineTo(offsetX + pt1.i, pt1.j + this.height / 2);
        ctx.lineTo(offsetX + pt1.i, pt1.j);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.beginPath();
        ctx.moveTo(offsetX + pt3.i, pt3.j);
        ctx.lineTo(offsetX + pt2.i, pt2.j);
        ctx.lineTo(offsetX + pt2.i, pt2.j + this.height / 2);
        ctx.lineTo(offsetX + pt3.i, pt3.j + this.height / 2);
        ctx.lineTo(offsetX + pt3.i, pt3.j);
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
    ...
    recoverSurface(canvas, diamond) {
        ...
        if (diamond.x == this.diamonds.length - 1) {
            ctx.beginPath();
            ctx.moveTo(offsetX + pt1.i, pt1.j);
            ctx.lineTo(offsetX + pt2.i, pt2.j);
            ctx.lineTo(offsetX + pt2.i, pt2.j + this.height / 2);
        ...
        if (diamond.y == this.diamonds.length - 1) {
            ctx.beginPath();
            ctx.moveTo(offsetX + pt3.i, pt3.j);
            ctx.lineTo(offsetX + pt2.i, pt2.j);
            ctx.lineTo(offsetX + pt2.i, pt2.j + this.height / 2);
...
function moveDetect(e) {
    ...
    if (i0 >= 0 && j0 >= 0) {
        if (i0 < board.diamonds.length && j0 < board.diamonds[i0].length) {
            var is_click = board.diamonds[i0][j0].state == "click";
            if (!is_click) {
                board.diamonds[i0][j0].state = "hover";
            }

            var is_last_coord_different = (i0 != last_i0 || j0 != last_j0) &&
                (last_i0 != -1 && last_j0 != -1);
            if (is_last_coord_different)
                if (board.diamonds[last_i0][last_j0].state == "hover") {
                    board.diamonds[last_i0][last_j0].state = "normal";
                    board.recoverSurface(canvas, board.diamonds[last_i0][last_j0]);

                    if (last_i0 + 1 < board.diamonds.length)
                        board.recoverSurface(canvas, board.diamonds[last_i0 + 1][last_j0]);
                    if (last_j0 + 1 < board.diamonds[last_i0].length)
                        board.recoverSurface(canvas, board.diamonds[last_i0][last_j0 + 1]);
                    if (last_i0 + 1 < board.diamonds.length &&
                        last_j0 + 1 < board.diamonds[last_i0 + 1].length)
                        board.recoverSurface(canvas, board.diamonds[last_i0 + 1][last_j0 + 1]);
                }
...
function pressDetect(e) {
    ...
    if (i0 >= 0 && j0 >= 0) {
        if (i0 < board.diamonds.length && j0 < board.diamonds[i0].length) {
            ...
            if (i0 + 1 < board.diamonds.length) {
                //board.diamonds[i0+1][j0].state = "normal";
                board.recoverSurface(canvas, board.diamonds[i0 + 1][j0]);
            }
            if (j0 + 1 < board.diamonds.length) {
                //board.diamonds[i0][j0+1].state = "normal";
                board.recoverSurface(canvas, board.diamonds[i0][j0 + 1]);
            }
            if (i0 + 1 < board.diamonds.length &&
                j0 + 1 < board.diamonds[i0 + 1].length) {
                //board.diamonds[i0+1][j0+1].state = "normal";
                board.recoverSurface(canvas, board.diamonds[i0 + 1][j0 + 1]);
            }
        }
    }
}